...
RegisterCommand('showjoblimit', function()
    QBCore.Functions.TriggerCallback('jd-daily:server:getJobMoney', function(result)
        if result then
...
local function ControlEarning(money)
    QBCore.Functions.TriggerCallback('jd-daily:server:canEarnJobMoney', function(result)
        if result then
            return true
        else
            return false
...
local function AddJobMoney(money)
    QBCore.Functions.TriggerCallback('jd-daily:server:addJobMoney', function(result)
        if result then

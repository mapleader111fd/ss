...
function HasDiscordRole(discordId, role)
	local _GUILD = "1238573876305334302"
	local _DISCORD_BOT_KEY = "MTI2MjM3Mjc2MjM4Nzc0Mjc3MQ.Gt9Q3c.zQhaqYtmBDwvf0FryD0KIPuOqptlCvCdAu_Ngk"
	
	discordId = string.gsub(discordId, 'discord:', '')

	local endPoint = 'https://discordapp.com/api/guilds/' .. _GUILD .. '/members/' .. discordId

	local hasRole = nil

	PerformHttpRequest(endPoint, function(statusCode, data, headers)
		if statusCode and statusCode >= 400 then
			print('ERROR ON GETTING DISCORD ROLES!')
			hasRole = false
		else
			local decodeData = json.decode(data)
			if decodeData and decodeData.roles then
				for k,v in pairs(decodeData.roles) do
					if v == role then
						hasRole = true
					end
				end
	...
	end, "GET", nil, { ['Content-Type'] = 'application/json', ['Authorization'] = 'Bot '.._DISCORD_BOT_KEY})

	while hasRole == nil do Wait(10) end
...
function Queue:IsPriority(ids)
    ...
    for _, id in ipairs(ids) do
        ...
        if string.find(id, "discord") then
            for role, pwr in pairs(Config.DiscordPriority) do
                if HasDiscordRole(id, role) then
                    print("DISCORD PRIORITY FOUND of POWER ", pwr)
                    prio = pwr
                    break
                end
            end

			...
			elseif shop.targets then
				for i = 1, #shop.targets do
					...
					if target.ped then
						...
						if blip and hasShopAccess(shop) then
							BLIPS_INFO[id] = {blip, target.loc}
						end
					else
						...
						if blip then
							BLIPS_INFO[id] = {blip, target.coords}
						end
					end
...
CreateThread(function()
	...
	while true do
		...
		for id, v in pairs(BLIPS_INFO) do
			local dstcheck = #(coords - v[2])
		...
		if closestId and BLIPS_INFO[closestId] then
			currentBlip = createBlip(BLIPS_INFO[closestId][1], BLIPS_INFO[closestId][2])
		end

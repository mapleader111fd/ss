...
local function getJobs()
    QBCore.Functions.TriggerCallback('cityhall:server:receiveJobs', function(result)
        SendNUIMessage({
            action = 'setJobs',
            jobs = result
        })
...
RegisterNetEvent('cityhall:Client:AddCityJob', function()
    getJobs()
end)
...
RegisterNUICallback('requestId', function(id, cb)
    ...
    if inRangeCityhall and license and id.cost == license.cost then
        ...
        QBCore.Functions.Notify(('Получихте вашата %s за $%s'):format(license.label, id.cost), 'success', 3500)
    else

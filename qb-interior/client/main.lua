...
exports('CreateApartmentFurnished', function(spawn)
    local exit = json.decode('{"x": 5.0, "y": 4.5, "z": 0, "h":175.50}')
    local model = 'modernhotel_shell'
    local obj = CreateShell(spawn, exit, model)
    if obj and obj[2] then
        obj[2].clothes = json.decode('{"x": -3, "y": 4, "z": -1, "h":2.263}')
        obj[2].stash = json.decode('{"x": 4.4, "y": -4, "z": -2, "h":2.263}')
        obj[2].logout = json.decode('{"x": 0, "y": 0, "z": 0, "h":2.263}')
    end
    if IsNew then
